...
class UsersHandler(BaseHandler):
    ...
    def get(self):
        ...
        users = self.db.query("select a.wid, a.username, b.orderdest from lem_webowner a left join lem_ivr_info b on a.wid = b.channel where a.channel = %s order by a.regtime desc", wid)
...
class PaymentHandler(BaseHandler):
    ...
    def get(self):
        ...
        if channel is None:
            entries = self.db.query("SELECT * FROM lez_webowner_bill where wid = %s ORDER BY billdate DESC LIMIT %s, %s", wid, start, pagesize)
        
            total = self.db.getint("SELECT count(*) from lez_webowner_bill where wid = %s", wid)
            turnpage = "admin/payment.html"
        else:
            entries = self.db.query("SELECT * FROM lez_webowner_channel_day where wid = %s and channel = %s ORDER BY subdate DESC LIMIT %s, %s", channel, wid, start, pagesize)
        
            total = self.db.getint("SELECT count(*) from lez_webowner_channel_day where wid = %s and channel = %s ", channel, wid)
...
class IVRDetailHandler(BaseHandler):
    ...
    def get(self):
        ...
        if channel is None:
            total = self.db.getint("select count(*) from lez_service_log where wid = %s and subtime >= %s and feeflag = 1", wid, today)
            entries = self.db.query("SELECT * FROM lez_service_log where wid = %s and subtime >= %s and feeflag = 1 ORDER BY subtime DESC LIMIT %s, %s", wid, today, start, pagesize)
        else:
            total = self.db.getint("select count(*) from lez_service_log where wid = %s and channel = %s and subtime >= %s and feeflag = 1", channel, wid, today)
            entries = self.db.query("SELECT * FROM lez_service_log where wid = %s and channel = %s and subtime >= %s and feeflag = 1 ORDER BY subtime DESC LIMIT %s, %s", channel, wid, today, start, pagesize)
...
def getLocalData(db, wid, channel):
    ...
    today = time.strftime('%Y-%m-%d',time.localtime(time.time()))
    if channel is None:
        showincome = db.get("SELECT sum(showincome) as showincome from lez_webowner_bill where wid = %s and payflag = 0", wid).showincome
        todayincome = db.get("SELECT sum(feeincome) as feeincome from lez_service_log where wid = %s and feeflag = 1 and subtime >= %s", wid, today).feeincome
    else:
        todayincome = db.get("SELECT sum(feeincome) as feeincome from lez_service_log where wid = %s and channel = %s and feeflag = 1 and subtime >= %s", channel, wid, today).feeincome
        showincome = 0
    paydata = dict(
...
class ModifyPwdHandler(BaseHandler):
    ...
    def post(self):
        ...
        if password is None or len(password) < 4:
            ...
            self.db.execute("update lem_webowner set password = %s where wid = %s", hashlib.md5(password).hexdigest(), wid)
            tipmessages = "密码修改成功."
